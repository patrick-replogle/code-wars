...
function presses(phrase) {
  ...
  let four = ['2', '3', '4', '5', '6', 'S', '8', 'Z' ]
  let five = ['7', '9']
  phrase = phrase.toUpperCase().split("");
  let count = 0;
  
  for(let i = 0; i < phrase.length; i++) {
    if(one.includes(phrase[i])) count += 1;
    if(two.includes(phrase[i])) count += 2;
    if(three.includes(phrase[i])) count += 3;
    if(four.includes(phrase[i])) count += 4;
    if(five.includes(phrase[i])) count += 5;
  }
  return count;
}
